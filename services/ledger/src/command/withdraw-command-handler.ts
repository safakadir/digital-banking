...
export class WithdrawCommandHandler {
  ...
  constructor(private readonly telemetry: TelemetryBundle) {
    ...
    this.inboxTableName = process.env.INBOX_TABLE || `LedgerSvc-CommandFn-Inbox-${process.env.ENV || 'dev'}`;
    this.ledgerTableName = process.env.LEDGER_TABLE_NAME || `LedgerSvc-LedgerTable-${process.env.ENV || 'dev'}`;
    this.balanceTableName = process.env.BALANCE_TABLE_NAME || `LedgerSvc-BalanceTable-${process.env.ENV || 'dev'}`;
    this.outboxTableName = process.env.OUTBOX_TABLE_NAME || `LedgerSvc-OutboxTable-${process.env.ENV || 'dev'}`;
  }
  ...
  async handle(command: WithdrawCommand): Promise<void> {
    ...
    try {
      ...
      if (error.name === 'ConditionalCheckFailedException') {
        ...
        if (cancellationReasons && cancellationReasons[3] && cancellationReasons[3].Code === 'ConditionalCheckFailed') {
          ...
          logger.info('Withdraw command handled successfully - insufficient funds (business rule)', {
            commandId: command.id,
            accountId: command.accountId,
        ...
        if (cancellationReasons && cancellationReasons[5] && cancellationReasons[5].Code === 'ConditionalCheckFailed') {
